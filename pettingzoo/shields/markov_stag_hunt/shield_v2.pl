...
safe_next :- hunt,      sensor(stag_near_other),   sensor(stag_near_self).
% do not hunt if stag is near and there is no other agent
safe_next :- \+hunt,    \+sensor(stag_near_other), sensor(stag_near_self).
% any action is fine if stag is not near
safe_next :- action(_), sensor(stag_near_other),   \+sensor(stag_near_self).
safe_next :- action(_), \+sensor(stag_near_other), \+sensor(stag_near_self).
